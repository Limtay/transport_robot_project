#ifndef ORIN_FIRMWARE_BRIDGE__RD_COMMAND_HPP_
#define ORIN_FIRMWARE_BRIDGE__RD_COMMAND_HPP_

// 커맨드 슬롯 매니저 (Code_modify.md §2, §3)
//  - 4개 슬롯, 각 5Hz 로 스케줄러가 발사 (slot 번호 낮을수록 우선순위 높음)
//  - duration: 0=forever / 1=once(RET_OK 까지 반복, **40회 시도** 후 포기) / 2~100 [sec]
//    once 만료가 시간이 아니라 시도 횟수인 이유는 00 C2 참조 — blackout(3s) 이
//    벽시계 timeout(2s) 을 먹어치워 REBOOT 직후 명령이 시도 0회로 죽던 버그.
//  - REBOOT: 응답 수신 후 해당 보드 3초 blackout + is_connected=false
//  - 우선순위 규칙(§2.3): auto command 요청 시 빈 슬롯 최상단 배치,
//    모두 차있으면 최하위 슬롯 일시정지 → 자리 차용 → 처리 후 복귀
//  - jeongae 자동 전개 시퀀스 FSM(§3) — DPC/카메라 단계는 레지스터 미확정으로 TODO

#include <array>
#include <atomic>
#include <chrono>
#include <mutex>
#include <string>
#include <vector>
#include "orin_firmware_bridge/rd_logger.hpp"
#include "orin_firmware_bridge/policy/rd_sequence.hpp"
#include "orin_firmware_bridge/core/rd_map.hpp"
#include "orin_firmware_bridge/policy/rd_command_catalog.hpp"

namespace orin_bridge {

// CommandSet.srv 의 inst 값과 1:1
enum class CmdInst : uint8_t {
    WRITE_REG  = 0,  // 기능1: 현재 reg 섀도 값 전송 (start_addr, data_len)
    WRITE_DATA = 1,  // 기능2: 사용자 데이터 전송 (start_addr, data[])
    READ       = 2,  // READ — 성공 시 reg 섀도 갱신 후 ILogger 로 출력 (A5: 이 계층은 rclcpp 를 모른다)
    REBOOT     = 3,  // 보드 reboot — 3초 접근 차단
};

// GET_STATUS(04 §4) 의 `slots[].cmd` — enum 은 문자열로 나간다.
inline const char* CmdInstName(uint8_t inst) {
    switch (static_cast<CmdInst>(inst)) {
        case CmdInst::WRITE_REG:  return "write_reg";
        case CmdInst::WRITE_DATA: return "write_data";
        case CmdInst::READ:       return "read";
        case CmdInst::REBOOT:     return "reboot";
        default:              return "unknown";
    }
}

constexpr uint16_t CMD_DURATION_FOREVER = 0;
constexpr uint16_t CMD_DURATION_ONCE    = 1;
// once 만료 기준 (00 C2). **시간이 아니라 시도 횟수다** — blackout(3s) 이 벽시계
// timeout(2s) 을 먹어치워 REBOOT 직후 명령이 시도 0회로 죽던 버그를 막는다.
// 5Hz 발사 기준 실제 시도 8초치.
constexpr uint32_t kOnceMaxAttempts     = 40;
constexpr uint16_t CMD_DURATION_MAX_SEC = 100;
constexpr uint8_t  CMD_SLOT_AUTO        = 255;  // SET 시 빈 슬롯 자동 선택
constexpr int      CMD_NUM_SLOTS        = 4;

// CommandSet.srv request 와 동일한 평면 구조 (서비스 의존성 없이 코어 분리)
// B6 — **의미 단위 명령**. 주소 번역은 브리지가 한다 (rd_command_catalog.hpp).
// `start_addr`/`data_len`/`data` 는 CMD_RAW_* 에서만 쓰인다.
struct CommandRequest_t {
    uint8_t  slot       = CMD_SLOT_AUTO;
    uint8_t  action     = 1;            // 0=RESET / 1=SET
    uint8_t  target_id  = 0;
    uint8_t  cmd        = 0;            // cmdcat::CMD_*
    std::vector<uint8_t> args;          // cmd 별 인자 (WRITE 계열: args[0] = 값)
    uint16_t start_addr = 0;            // CMD_RAW_* 전용
    uint16_t data_len   = 0;            // CMD_RAW_READ 전용
    std::vector<uint8_t> data;          // CMD_RAW_WRITE 전용
    uint16_t duration   = CMD_DURATION_ONCE;
};

class RdCommand : public ISlotHost {
public:
    explicit RdCommand(RobotState_t* state);

    // 로거 주입 (A5). 미주입이면 DefaultLogger 로 폴백.
    void SetLogger(ILogger* lg) { log_ = lg; }

    // ===== ROS 스레드에서 호출 =====
    bool HandleRequest(const CommandRequest_t& req, std::string* out_msg);
    // jeongae 는 **RdSequence 가 소유한다** (02 A3). 여기서는 기존 호출부가 깨지지 않도록
    // 위임만 한다 — 슬롯 관리자와 전개 시퀀스는 다른 관심사다.
    void TriggerJeongae()            { seq_.Trigger(); }
    void SetJeongaeLock(bool lock)   { seq_.SetLock(lock); }
    bool GetJeongaeLock() const      { return seq_.GetLock(); }
    RdSequence& Sequence()           { return seq_; }

    // ── ISlotHost (RdSequence 가 슬롯을 빌려 쓰는 통로) ──
    bool PostAutoWriteTo(uint8_t target, uint16_t addr, uint8_t value) override;
    bool AutoCommandDone(bool* ok) const override;
    // DPC 섀도의 sys_state. **읽은 적이 없으면 false** — 값(0=CTRL)만으로는
    // 미판독과 정상 대기를 구분할 수 없다 (rd_sequence.hpp 참조).
    bool DpcSysState(uint8_t* out) const override;
    void SetCmdVelPaused(bool paused) override { cmd_vel_paused_.store(paused); }
    bool IsCmdVelPaused() const override { return cmd_vel_paused_.load(); }

    // ===== 스케줄러 스레드에서 호출 =====
    // 슬롯 차례에 발사할 task 를 꺼낸다. false = 이번 차례 skip (빈 슬롯/만료/blackout)
    bool GetSlotTask(int slot, TaskConfig_t* out_task);
    // 트랜잭션 결과 보고 (tx→rx→decode 전체 결과)
    void ReportResult(int slot, RD_RET ret);
    // REBOOT 후 3초간 해당 보드 전체 접근 차단
    bool IsTargetBlackedOut(uint8_t target_id);
    // jeongae 시퀀스 soft ESTOP 동안 50Hz cmd_vel WRITE 정지 (§3.2)

    // GET_STATUS(04 §4) 용 슬롯 스냅샷. 내부 Slot_t 를 그대로 내보내지 않는 이유는
    // 그것이 스케줄 스레드가 만지는 살아 있는 상태이기 때문이다 — 값만 복사해 넘긴다.
    struct SlotView_t {
        bool     active   = false;
        uint8_t  target_id = 0;
        uint8_t  inst      = 0;
        uint16_t duration  = 0;
        uint32_t attempts  = 0;
    };
    std::vector<SlotView_t> SlotSnapshot() const;

    // 마지막으로 **성공한** 슬롯 READ 가 무엇을 덮었는가 (07 §2 Tab3).
    //
    // 왜 필요한가: 레지스터 표를 그리려면 어느 바이트가 신선한지 알아야 한다. 섀도는
    // 언제나 값을 갖고 있으므로, 이것 없이 그리면 **한 번도 읽은 적 없는 자리의 0 이
    // 방금 읽은 값처럼 보인다** — 이 프로젝트가 반복해서 지워 온 결함 형태다.
    // 값 자체는 여기 담지 않는다: 질의 시점의 섀도가 언제나 더 신선하다.
    struct LastRead_t {
        bool     valid = false;
        uint8_t  target_id = 0;
        ReadSpanEntry spans[kMaxReadSegs] = {};
        uint8_t  count = 0;
        double   age_s = 0.0;      // 스냅샷을 뜬 시점 기준 경과
    };
    LastRead_t LastReadSnapshot() const;
    // 자동 시퀀스 FSM tick — 스케줄러 루프에서 주기 호출
    void TickAutoSequence() { seq_.Tick(); }   // 스케줄러가 5Hz 로 호출 (위임)

private:
    struct Slot_t {
        bool     active     = false;
        bool     is_auto    = false;   // auto sequence 가 차용한 슬롯
        uint8_t  target_id  = 0;
        CmdInst  inst       = CmdInst::READ;
        // 의미 단위 READ 의 구간 (B6). nullptr 이면 start_addr/data_len 단일 구간이다 —
        // raw 와 auto sequence 가 그 길을 쓴다.
        const ReadPreset* read_span = nullptr;
        uint8_t  cmd        = 0;       // 로그·스냅샷용 (cmdcat::CMD_*)
        uint16_t start_addr = 0;
        uint16_t data_len   = 0;
        std::vector<uint8_t> user_data;
        uint16_t duration   = CMD_DURATION_ONCE;
        std::chrono::steady_clock::time_point start_time;
        uint32_t err_streak = 0;       // 연속 실패 수 (forever/N초 로그 억제용)
        uint32_t attempts   = 0;       // **실제 발사 횟수** — once 만료 판정 기준 (00 C2)
    };


    ILogger*      log_ = nullptr;
    RobotState_t* state_;
    mutable std::mutex mutex_;   // slots_ / saved_slot_ / seq 상태 보호 (ROS ↔ sched 스레드)
                                 // (mutable: const 인 SlotSnapshot() 이 잠근다)

    std::array<Slot_t, CMD_NUM_SLOTS> slots_;

    // 우선순위 규칙: auto 가 차용하며 일시정지된 원래 슬롯 백업
    Slot_t saved_slot_;
    int    saved_slot_idx_ = -1;

    // REBOOT blackout
    std::array<std::chrono::steady_clock::time_point, 3> blackout_until_{}; // ECU/DPC/PCU
    static int TargetIndex(uint8_t target_id);

    // jeongae 시퀀스 상태
    // 자동 슬롯(시퀀스가 빌린 자리)의 결과. **슬롯 관리자의 상태**이므로 여기 남는다 —
    // 시퀀스는 AutoCommandDone() 으로 물어보기만 한다.
    // 마지막 성공 READ (Tab3 신선도 판정). 스케줄 스레드가 쓰고 ROS 스레드가 읽는다.
    LastRead_t last_read_;
    std::chrono::steady_clock::time_point last_read_time_;

    int  seq_slot_     = -1;
    bool seq_cmd_done_ = false;
    bool seq_cmd_ok_   = false;

    RdSequence seq_{this};
    std::atomic<bool> cmd_vel_paused_{false};

    // 내부 헬퍼 (mutex_ 잡은 상태에서 호출)
    bool SetSlotLocked(int idx, const CommandRequest_t& req, std::string* out_msg);
    void ClearSlotLocked(int idx);
    int  AcquireAutoSlotLocked();   // §3.1: 빈 칸 최상단, 없으면 최하위 일시정지
    void ReleaseAutoSlotLocked(int idx);

    static const char* TargetName(uint8_t target_id);
};

} // namespace orin_bridge

#endif
