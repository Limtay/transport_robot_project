...
constexpr uint16_t REG_DATA_FB_OFFSET    =  96;
constexpr uint16_t REG_DATA_FB_SIZE      =   4;
...
/* ===== [FEEDBACK] addr 96~99 (4 bytes) — RX from DPC ===== */
typedef struct __attribute__((packed)) {
    /* addr 96 */ uint8_t fb_jeongae;   // 0=open / 1=closed
    /* addr 97 */ uint8_t reserved[3];
} DATA_FB_t;

/* ===== [CMD] addr 128~131 (4 bytes) — TX to DPC ===== */
typedef struct __attribute__((packed)) {
    /* addr 128 */ uint8_t cmd_jeongae; // 0=open / 1=close
...
typedef struct __attribute__((packed)) {
    ...
    /* addr  96 */ DATA_FB_t  fb;
    /* addr 100 */ uint8_t    reserved2[28];
    /* addr 128 */ CMD_t      cmd;
...
static_assert(sizeof(DATA_FB_t)  == REG_DATA_FB_SIZE,  "DATA_FB_t size mismatch");
