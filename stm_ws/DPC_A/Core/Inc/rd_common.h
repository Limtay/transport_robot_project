 ...
 *   - 상위 FSM (robot_state, SYSTEM_STATE_e) 가 "왜 시스템이 멈췄나" 를
 * 표현하고, 채널 lifecycle 은 "통신/채널 자체가 살아있나" 만 표현한다.
 *   - 따라서 IDLE / PAUSED / SUSPENDED 같은 "외부 요청 정지" 상태는 두지 않는다
 * — 모두 상위 FSM (ESTOP_SW, MANUAL 등) 의 책임 영역.
 *   - UNINIT 폐기: memset 0 == LS_INIT 으로 자연 초기화. 미구현 모듈 (예: IMU)
 * 은 LS_OFFLINE 으로 명시 발행.
 *
 *      | 값  | 상수          | 설명
 * ----+-----+--------------+----------------------------------------- */
...
#define LS_DEGRADED                                                            \
  3 /* 통신 동작 중이나 최근 에러율 높음 (decay 카운터 임계 초과) */
#define LS_RECOVERING                                                          \
  4 /* 상위 명령으로 복구 시퀀스 실행 중 (이후 LS_INIT 으로 전이) */
/* 5~14 reserved */
 ...
 *   진입:              LS_RUNNING  && cnt > DEGRADED_THRESHOLD_HIGH →
 * LS_DEGRADED 복귀:              LS_DEGRADED && cnt < DEGRADED_THRESHOLD_LOW  →
 * LS_RUNNING (4× 히스테리시스 갭으로 flapping 방지)
 *
 * 설계 기준 (Checker @ 100Hz):
 *   - 경계 손실률 = 5%   (이 이하는 cnt 자연 감쇠로 0 수렴, DEGRADED 진입 안
 * 함)
 *   - 10% 손실           → ~2.0 sec 후 LS_DEGRADED 진입
 *   - 100% 손실 (단절)   → ~0.10 sec 진입, ~0.5 sec CNT_MAX 포화
 *   - 에러 정지 후 복귀:  cnt=200 → 1.5 sec, cnt=1000 → 9.5 sec
 *
 * 채널 통신 주파수별 K (식: K = 2000 / F_comm — 어느 F_comm 이든 5% 손실 =
 * 중립):
 ...
 * 심각도 그룹: 정상(0) | 정보(1) | 경고(2~6) | 에러(7~10) | 심각(11~13) |
 * 치명(14~15)
 * -----------------------------------------------------------------------
 *      | 값                | 상수 |    설명                            | 심각도
 * ----+----------------+-----+-------------------------------+---------*/
...
typedef struct {
  STATE_t
      state; /**< lifecycle(bits[3:0]) + health(bits[7:4]) — LS_*|HC_* 참조.
              *  lifecycle 전이 ownership:
              *    INIT     → LS_READY
              *    Checker  → LS_INIT/LS_READY → LS_RUNNING 첫 승격
              *               LS_RUNNING ↔ LS_DEGRADED  (degraded_cnt
              히스테리시스)
              *               LS_OFFLINE 강제 (HC_FATAL 또는 rx_error_cnt 임계)
              *    RECOVERY → LS_RECOVERING (진입 표시, 성공 시 INIT 가 LS_READY
              로 reset, 실패 시 LS_OFFLINE 으로 강제 전이)
              *  IDLEHandler/Transmit/RX_Apply 등은 lifecycle 직접 변경 금지. */
  uint32_t
      isr_err_code; /**< HAL_*_ErrorCallback 에서 캡처한 raw HAL 에러코드 */
  uint16_t rx_error_cnt; /**< 연속 HAL/packet 에러 누적 카운터 (FATAL OFFLINE
                            판정용, 단조 증가) */
  uint16_t tx_error_cnt; /**< TX 에러 누적 카운터 (디버그용)        */
  uint16_t degraded_cnt; /**< LS_DEGRADED 산출용 decay 카운터.
                                   매 에러시 +degraded_k (Checker 인자) / 무에러
                            tick 시 -DEGRADED_TICK_DECAY.
                              DEGRADED_THRESHOLD_HIGH 초과 → LS_DEGRADED 진입,
                              DEGRADED_THRESHOLD_LOW 미만 → LS_RUNNING 복귀
                            (히스테리시스 4× 갭). DEGRADED_CNT_MAX 포화. */
} ERROR_STATUS_t;
