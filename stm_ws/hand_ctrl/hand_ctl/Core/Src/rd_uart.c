...
RD_RET RD_UART_INIT(UART_Ring_t *uart_obj)
{
	...
	/*
	// 1. 부팅 직후 혹시 고여있을지 모르는 DR 레지스터 값을 강제로 읽어서 폐기 (RXNE 제거)
	volatile uint32_t tmpreg = uart_obj->huart->Instance->DR;
	(void)tmpreg; // 컴파일러 경고 방지용

	// 2. 고질적인 하드웨어 에러 플래그들(Overrun, Noise, Frame) 강제 전원 차단 및 청소
	//__HAL_UART_CLEAR_FLAG(uart_obj->huart, UART_CLEAR_ORE | UART_CLEAR_FE | UART_CLEAR_PE);

	// 3. IDLE 플래그 한 번 더 확실하게 청소
	...
	*/
    ...
    HAL_StatusTypeDef state = HAL_UART_Receive_DMA(uart_obj->huart, uart_obj->rx_buffer, RX_BUFFER_SIZE);
    if (state != HAL_OK) return RET_NOK;
    return RET_OK;
}
...
RD_RET RD_UART_IDLEHandler(UART_Ring_t *uart_obj)
{
    ...
    // RS485 Thread 깨우기
    if (uart_obj->huart->Instance == USART1) osThreadFlagsSet(rs485TaskHandle, 0x0001);
#endif
...
RD_RET RD_RS485_Transmit(RS485_t *rs485_obj)
{
    ...
    rs485_obj->tx_mode = 1;
    rs485_obj->last_tx_tick = HAL_GetTick();
    for(volatile int i = 0; i < 10; i++); // delay margin
    /*=================================*/
    /* stm32f4xx_it.c
